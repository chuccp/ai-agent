...
func BuildExecutionLayers(nodeMap map[string]node.Node, endNode node.Node) ([][]node.Node, error) {
	...
	createNodeTrees(endNode, nodeMap, execNodes)
...
func createNodeTrees(n node.Node, nodeMap map[string]node.Node, execNodes *ExecNodes) {
	if !execNodes.Contains(n.GetID()) {
		...
		for _, prevNodeID := range execNode.GetPrevNodeIDs() {
			...
			if !ok {
				panic("Unknown dependency node: " + prevNodeID)
			}
			createNodeTrees(prevNode, nodeMap, execNodes)
		}
	}
}
